...
@app.get('/jobResults')
def jobResults(jobIDs: Annotated[List[str], Query(description="A list of JobIDs. Returned from `/crawl`.")]):
    """Get the job status and position in queue"""
    results = {}
    for jobId in jobIDs:
        try:
            ...
            if job.get_status() != "finished":
                results[jobId] = HTTPException(status_code=102, detail="job is not finished yet, use /jobStatus endpoint to get job status and wait until finished to try again")
            ...
            if result.type == result.Type.SUCCESSFUL:
                ...
                results[jobId] = HTTPException(status_code=500, detail="the job exists and is flagged finished, but it did not succeed")
        except Exception:
            results[jobId] = HTTPException(status_code=500, detail="Failed to get job result - unknown error.")
    return results
